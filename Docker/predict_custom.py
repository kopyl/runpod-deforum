...
class Predictor(BasePredictor):
    ...
    def predict(
        self,
        aws_access_key_id: str = Input(default="", description="AWS Access Key ID",),
        aws_secret_access_key: str = Input(
            default="", description="AWS Secret Access Key",
        ),
        aws_bucket_name: str = Input(default="", description="AWS Bucket Name",),
        endpoint_url: str = Input(
            default="https://s3.amazonaws.com/", description="AWS Endpoint URL",
        ),
        ...
        padding_mode: str = Input(
            choices=["border", "reflection", "zeros"], default="border",
        ),
        sampling_mode: str = Input(
            choices=["bicubic", "bilinear", "nearest"], default="bicubic",
        ),
        ...
        hybrid_video_motion: str = Input(
            choices=["None", "Optical Flow", "Perspective", "Affine"], default="None",
        ),
        hybrid_video_flow_method: str = Input(
            choices=["Farneback", "DenseRLOF", "SF"], default="Farneback",
        ),
        ...
        hybrid_video_comp_mask_equalize: str = Input(
            choices=["None", "Before", "After", "Both"], default="None",
        ),
        ...
        # create empty file to allow handler to return
        open("/tmp/finished", "a").close()
